...
pub use storage::{CacheStorage, CacheEntry};
...
impl CacheManager {
    ...
    pub fn get_issue_context(&self, repo: &str, issue_number: u32) -> Result<Option<IssueContext>> {
        ...
        let path = self.cache_dir.join("contexts").join(format!("{}.json", key));
        
        if path.exists() {
            let data = fs::read(&path)
                .with_context(|| format!("Failed to read context cache: {:?}", path))?;
            
            let context: IssueContext = serde_json::from_slice(&data)
                .context("Failed to deserialize issue context")?;
    ...
    /// Cache issue context
    pub fn cache_issue_context(&self, repo: &str, issue_number: u32, context: &IssueContext) -> Result<()> {
        let key = format!("{}_{}", repo.replace('/', "_"), issue_number);
        let path = self.cache_dir.join("contexts").join(format!("{}.json", key));
        
        let data = serde_json::to_vec_pretty(context)
            .context("Failed to serialize issue context")?;
