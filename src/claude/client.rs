...
use crate::claude::{MessagesRequest, MessagesResponse, ErrorResponse, get_api_key, resolve_model_alias};
...
impl ClaudeClient {
    ...
    pub fn messages_with_retry(&self, request: MessagesRequest, max_retries: u32) -> Result<MessagesResponse> {
        match self {
...
impl RealClaude {
    ...
    /// Send a messages request with retries
    pub fn messages_with_retry(&self, mut request: MessagesRequest, max_retries: u32) -> Result<MessagesResponse> {
        let mut attempts = 0;
...
mod tests {
    use super::*;
    use crate::claude::{Message, Content, Usage};
    
    #[test]
    fn test_mock_claude_client() {
        let mock_response = MessagesResponse {
            id: "msg_123".to_string(),
            content: vec![Content::Text { text: "Test response".to_string() }],
        ...
        let client = MockClaude::new()
            .with_response(mock_response.clone());
