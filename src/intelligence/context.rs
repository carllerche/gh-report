use crate::config::{Label, Importance};
use crate::intelligence::{PrioritizedIssue, ActionItem, Urgency};
...
pub fn build_context_prompt(
    ...
    if !labels.is_empty() {
        ...
        for label in labels {
            prompt.push_str(&format!("### {} ({})\n", label.name, format_importance(label.importance)));
            prompt.push_str(&format!("{}\n", label.context));
    ...
    if !repo_importances.is_empty() {
        ...
        for importance in [Importance::Critical, Importance::High, Importance::Medium, Importance::Low] {
    ...
    prompt.push_str(r#"## Summarization Guidelines
...
fn determine_urgency(issue: &PrioritizedIssue) -> Urgency {
    ...
    if issue.matched_rules.iter().any(|r| r.rule_type == "security_issues") {
    ...
    if issue.matched_rules.iter().any(|r| 
        r.rule_type == "breaking_changes" || 
        r.rule_type == "review_requests"
    ) {
        return Urgency::High;
    }
...
fn generate_action(issue: &crate::github::Issue, matched_rules: &[crate::intelligence::MatchedRule]) -> Option<String> {
    // Security issues
    if matched_rules.iter().any(|r| r.rule_type == "security_issues") {
        return Some(format!(
    ...
    // Review requests
    if matched_rules.iter().any(|r| r.rule_type == "review_requests") {
        return Some(format!("Review PR #{}", issue.number));
    }
    
    // Breaking changes
    if matched_rules.iter().any(|r| r.rule_type == "breaking_changes") {
        return Some(format!(
    ...
    if issue.is_pull_request {
        Some(format!("Review PR #{}", issue.number))
    } else if issue.labels.iter().any(|l| 
        l.name.to_lowercase().contains("bug") || 
        l.name.to_lowercase().contains("urgent")
    ) {
        Some(format!("Address issue #{}", issue.number))
    } else {
        None
    }
}
...
mod tests {
    ...
    use crate::intelligence::{PriorityScore, MatchedRule};
    use jiff::Timestamp;
