...
impl<'a> RepositoryDiscovery<'a> {
    ...
    pub fn discover_active_repos(&self, lookback_days: u32) -> Result<Vec<DiscoveredRepo>> {
        ...
        let involves_query = format!("involves:{} updated:>{}", 
            username, 
    ...
    fn enrich_repo_metrics(&self, repo: &mut DiscoveredRepo, lookback_days: u32) -> Result<()> {
        ...
        info!("Metrics for {}: commits={}, prs={}, issues={}, comments={}", 
            repo.full_name, repo.metrics.commits, repo.metrics.prs, 
            repo.metrics.issues, repo.metrics.comments);
