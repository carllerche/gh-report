...
impl<'a> RepositoryDiscovery<'a> {
    ...
    pub fn discover_active_repos(&self, lookback_days: u32) -> Result<Vec<DiscoveredRepo>> {
        ...
        // Get current user
        let username = self.client.get_current_user()
        ...
        // 1. Repositories where user has recently created issues/PRs
        let author_query = format!("author:{} updated:>{}", 
            username, 
            days_ago_date(lookback_days)
        );
        repos.extend(self.search_repos(&author_query)?);
        
        // 2. Repositories where user was mentioned
        let mention_query = format!("mentions:{} updated:>{}", 
            username,
            days_ago_date(lookback_days)
        );
        repos.extend(self.search_repos(&mention_query)?);
        
        // 3. Repositories where user has been assigned
        let assignee_query = format!("assignee:{} updated:>{}", 
            username,
            days_ago_date(lookback_days)
        );
        repos.extend(self.search_repos(&assignee_query)?);
        
        // 4. Repositories where user has reviewed PRs
        let review_query = format!("reviewed-by:{} updated:>{}", 
            username,
            days_ago_date(lookback_days)
        );
        ...
        for repo in repos {
            unique_repos.entry(repo.full_name.clone())
    ...
    fn search_repos(&self, query: &str) -> Result<Vec<DiscoveredRepo>> {
        ...
        // Real implementation uses gh search
        {
                ...
                let output = Command::new("gh")
                    ...
                    .output()
                    .context("Failed to execute gh search")?;
    ...
    fn enrich_repo_metrics(&self, repo: &mut DiscoveredRepo, lookback_days: u32) -> Result<()> {
        ...
        match self.client.fetch_issues(&repo.full_name, Some(since)) {
            ...
            Err(e) => {
                debug!("Failed to fetch issues for {}: {}", repo.full_name, e);
            }
        }
