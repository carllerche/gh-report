...
pub fn group_activities_by_repo(issues: Vec<Issue>) -> BTreeMap<String, RepoActivity> {
    ...
    for issue in issues {
        ...
        let activity = activities.entry(repo_name).or_insert_with(RepoActivity::default);
...
mod tests {
    ...
    fn test_extract_repo_from_url() {
        ...
        assert_eq!(
            extract_repo_from_url("https://example.com/foo/bar"),
            None
        );
    }
